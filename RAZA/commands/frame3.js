...
module.exports.onLoad = async () => {
    ...
    const path = resolve(__dirname, 'cache/canvas', 'frame3.png'); 
    if (!existsSync(dirMaterial)) mkdirSync(dirMaterial, { recursive: true });

    // 👇 yahan apna frame ka PNG/JPG link dal do
    if (!existsSync(path)) 
        await downloadFile("https://i.ibb.co/0ybzRgHN/20250824-175315.jpg", path);
};
...
async function makeImage(uid) {
    ...
    // Base frame
    let frame_img = await jimp.read(__root + "/frame3.png");
    ...
    frame_img.composite(circleAvt.resize(195, 195), 240, 240);  
