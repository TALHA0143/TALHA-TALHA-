...
module.exports.run = async function({ api, args, Users, event, Threads, utils, client }) {
	...
	if (!info.adminIDs.some(item => item.id == api.getCurrentUserID())) return api.sendMessage(formatMessage('The bot needs group admin rights to use this command\nPlease add and try again!'), threadID, messageID);
  ...
  */
  ...
  if(args[0] == "view") {
  	if(!args[1]) {
  		...
  		if(!mywarn) return api.sendMessage(formatMessage('✅You have never been warned'), threadID, messageID);
  		var num = 1;
  		for(let reasonwarn of mywarn) {
  			msg += `reasonwarn\n`;
  		}
  		api.sendMessage(formatMessage(`❎You have been warned for the reason : ${msg}`), threadID, messageID);
  	}
  		...
  		allwarn == "" ? api.sendMessage(formatMessage("✅No one in your group has been warned yet"), threadID, messageID) : api.sendMessage(formatMessage("List of members who have been warned:\n"+allwarn, threadID, messageID);
  	}
  }
	...
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎Right cunt border!', threadID, messageID);
  	...
  	bans.banned;
  	if(!mybox.includes(id)) return api.sendMessage(formatMessage("✅This person hasn't been banned from your group yet"), threadID, messageID);
			api.sendMessage(formatMessage(`✅Removed the member with id ${id} from the group banned list`), threadID, messageID);
  	...
  	msg == "" ? api.sendMessage(formatMessage("✅No one in your group has been banned from the group yet"), threadID, messageID) : api.sendMessage(formatMessage("❎Members who have been banned from the group:\n"+msg, threadID, messageID);
  }
	...
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎Right cunt border!', threadID, messageID);
  	...
  	api.sendMessage("Reset all data in your group"), threadID, messageID);
       ...
       var info = await api.getThreadInfo(threadID);
	if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage(formatMessage('Right cunt border!'), threadID, messageID);
  var reason = "";
