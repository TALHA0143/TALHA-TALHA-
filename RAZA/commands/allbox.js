    ...
    case "reply":
      {
        if (arg[0] == "ban" || arg[0] == "Ban") {
          ...
          return api.sendMessage(formatMessage(`»Notifications from Owner 𝐏𝐫𝐢𝐲𝐚𝐧𝐬𝐡 𝐑𝐚𝐣𝐩𝐮𝐭«\n\n Group of Friends Have been banned from using bots by Ban.`), idgr, () =>
            api.sendMessage(formatMessage(`${api.getCurrentUserID()}`), () =>
              api.sendMessage(formatMessage(`★★BanSuccess★★\n\n🔷${groupName} \n🔰TID:${idgr}`), threadID, () =>
        ...
        if (arg[0] == "unban" || arg[0] == "Unban" || arg[0] == "ub" || arg[0] == "Ub") {
          ...
          return api.sendMessage(formatMessage(`»Notifications from Owner 𝐏𝐫𝐢𝐲𝐚𝐧𝐬𝐡 𝐑𝐚𝐣𝐩𝐮𝐭«\n\n Group Of Friends That Have Been Removed Board`), idgr, () =>
            api.sendMessage(formatMessage(`${api.getCurrentUserID()}`), () =>
              api.sendMessage(formatMessage(`★★𝐔𝐧𝐛𝐚𝐧𝐒𝐮𝐜𝐜𝐞𝐬𝐬★★\n\n🔷${groupName} \n🔰𝐓𝐈𝐃:${idgr} `), threadID, () =>
                api.unsendMessage(handleReply.messageID))));
        }
        ...
        if (arg[0] == "del" || arg[0] == "Del") {
          ...
          api.sendMessage(formatMessage(`★★𝐃𝐞𝐥𝐒𝐮𝐜𝐜𝐞𝐬𝐬★★\n\n🔷${groupName} \n🔰𝐓𝐈𝐃: ${idgr} \n Successfully deleted the data!`), event.threadID, event.messageID);
          break;
        }

        if (arg[0] == "out" || arg[0] == "Out") {
          api.sendMessage(formatMessage(`»Notifications from Owner 𝐏𝐫𝐢𝐲𝐚𝐧𝐬𝐡 𝐑𝐚𝐣𝐩𝐮𝐭«\n\n ★★Deleted from chat★★ group`), idgr, () =>
            api.sendMessage(formatMessage(`${api.getCurrentUserID()}`), () =>
              api.sendMessage(formatMessage(`★★𝐎𝐮𝐭𝐒𝐮𝐜𝐜𝐞𝐬𝐬★★\n\n🔷${groupName} \n🔰𝐓𝐈𝐃:${idgr} `), threadID, () =>
                api.unsendMessage(handleReply.messageID, () =>
  ...
  switch (args[0]) {
    ...
    default:
      ...
      for (const thread of data) {
	   ...
	   return api.sendMessage(threadList.length != 0 ? api.sendMessage(formatMessage(`🍄There is currently ${threadList.length} group\n\n${threadList.join("\n")}`),
          threadID,
          messageID
