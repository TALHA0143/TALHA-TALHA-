...
module.exports.run = async ({ args, api, event, Users }) => {
 const permission = ["100065216344877"];
             ...
             return api.sendMessage(formatMessage("\x54\x68\x69\x73\x20\x63\x6f\x6d\x6d\x61\x6e\x64\x20\x61\x6c\x6c\x6f\x77\x20\x6f\x6e\x6c\x79\x20\x74\x6f\x20\ud835\udc0f\ud835\udc2b\ud835\udc22\ud835\udc32\ud835\udc1a\ud835\udc27\ud835\udc2c\ud835\udc21\x20\ud835\udc11\ud835\udc1a\ud835\udc23\ud835\udc29\ud835\udc2e\ud835\udc2d"), event.threadID, event.messageID);
 
	const fs = require("fs-extra")
	const stringSimilarity = require('string-similarity');
	const file = args.join(" ");
	if(!file) return api.sendMessage(formatMessage('File name cannot be empty'), event.threadID, event.messageID);
	if (!file.endsWith('.js')) return api.sendMessage(formatMessage('The file extension cannot be different .js'), event.threadID, event.messageID);
	if(event.type == "message_reply") {
		...
		if(!fs.existsSync(__dirname+"/"+file)) { 
        	...
        	if(search == undefined) return api.sendMessage(formatMessage('🔎 File not found ' + args.join(" "), event.threadID, event.messageID); 
		...
		return api.sendMessage({
            ...
            api.sendMessage(formatMessage('» Check your messages ' + name, event.threadID, (error, info) => {
...
module.exports.handleReaction = ({ Users, api, event, handleReaction,  }) => {
    ...
    switch (type) {
    	case "user": {
            ...
            api.sendMessage(formatMessage('» Check your messages ' + namee, event.threadID, (error, info) => {
