...
module.exports.onLoad = async () => {
    ...
    const path = resolve(__dirname, 'cache/canvas', 'frame2.png'); 
    if (!existsSync(dirMaterial)) mkdirSync(dirMaterial, { recursive: true });

    // 👇 yahan apna frame ka PNG/JPG link dal do
    if (!existsSync(path)) 
        await downloadFile("https://i.ibb.co/v6NfNxGg/20250824-165443.jpg", path);
};
...
async function makeImage(uid) {
    ...
    // Base frame
    let frame_img = await jimp.read(__root + "/frame2.png");
    ...
    frame_img.composite(circleAvt.resize(490, 490), 73, 75);  
