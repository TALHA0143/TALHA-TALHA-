...
if (!args[0]) return api.sendMessage(formatMessage(`[⚜️]=== 𝗙𝗔𝗖𝗘𝗕𝗢𝗢𝗞 𝗔𝗩𝗧 ===[⚜️]\n\n[⚜️]→ ${prefix}${mn} box is get avt your group\n\n[⚜️]→ ${prefix}${mn} id [id to get] <get the image of the uid of the people>\n\n[⚜️]→ ${prefix}${mn} link [link to get] <get the link of your people>\n\n[⚜️]→ ${prefix}${mn} user <the empty command is the get avatar of your user user>\n\n[⚜️]→ ${prefix}${mn} user [@mentions] <get avatar people tagged>`),event.threadID,event.messageID);
  if (args[0] == "box") {
      ...
      return request(encodeURI(`${threadInfo.imageSrc}`)).pipe(fs.createWriteStream(__dirname+'/cache/1.png')).on('close',() => callback());
          ...
          if(!img) api.sendMessage(formatMessage(`[⚜️]→ AVATAR your box ${threadInfo.threadName} here`),event.threadID,event.messageID)
  ...
  try {
  var id = args[1];
  if (!id) return api.sendMessage(formatMessage(`[⚜️]→ Please enter uid to get avatar.`),event.threadID,event.messageID);
  ...
  api.sendMessage(formatMessage(`[⚜️]→ Can't get photo user.`),event.threadID,event.messageID);
 }
}
else if (args[0] == "link") {
var link = args[1];
if (!link) return api.sendMessage(formatMessage(`[⚜️]→ Please enter link to get avatar.`),event.threadID,event.messageID);
...
return request(encodeURI(`https://graph.facebook.com/${id}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`)).pipe(fs.createWriteStream(__dirname+'/cache/1.png')).on('close',() => callback());
    ...
    api.sendMessage(formatMessage("[⚜️]→ User does not exist."),event.threadID,event.messageID)
}
}
else if(args[0] == "user") {
  if (!args[1]) {
    ...
    api.sendMessage(formatMessage(`[⚜️]→ To install Sai order. Enter ${prefix}${mn} to see your module's commands..`),event.threadID,event.messageID);
  }
}
else {
  api.sendMessage(formatMessage(`[⚜️]→ To install Sai order. Enter ${prefix}${mn} to see your module's commands..`),event.threadID,event.messageID);
